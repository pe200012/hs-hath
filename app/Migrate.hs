...
import           Database                ( FileRecord(..), initializeDB )
...
parseOptions = MigrateOptions
  <$> option parseSourceType
      ( long "source-type"
     <> short 't'
     <> metavar "TYPE"
     <> help "Source type: sqlite or filesystem" )
  <*> strOption
      ( long "source"
     <> short 's'
     <> metavar "PATH"
...
main = do
  ...
  let r2Cfg = R2Config
            { r2Endpoint = optR2Endpoint opts
            , r2Bucket   = optR2Bucket opts
            }
    ...
    if optDryRun opts
      then do
        when (optVerbose opts) $
          TIO.putStrLn $ "  Would upload: " <> fileId
      else do
        ...
        case result of
          Left err -> do
            ...
            when (optVerbose opts) $
              TIO.putStrLn $ "  Error uploading " <> fileId <> ": " <> show err
          Right () -> do
            atomicModifyIORef' migratedCount (\c -> (c + 1, ()))
            when (optVerbose opts) $
              TIO.putStrLn $ "  Uploaded: " <> fileId
    
    -- Progress update every batch
    let idx' = _idx + 1
    when (idx' `mod` optBatchSize opts == 0) $
      TIO.putStrLn $ "  Progress: " <> show idx' <> "/" <> show totalCount
        ...
        when isFile $ do
          if optDryRun opts
            then do
              ...
              result <- try @SomeException $ Minio.runMinioWith (r2MinioConn r2Conn) $
              ...
              case result of
                ...
                Right (Left mErr) -> do
                  atomicModifyIORef' errorCount (\c -> (c + 1, ()))
                  when (optVerbose opts) $
                    TIO.putStrLn $ "  Error uploading " <> fileId <> ": " <> show mErr
                Right (Right ()) -> do
                  atomicModifyIORef' migratedCount (\c -> (c + 1, ()))
                  when (optVerbose opts) $
                    TIO.putStrLn $ "  Uploaded: " <> fileId
...
uploadToR2 conn uri record = do
  ...
  result <- try @SomeException $ Minio.runMinioWith (r2MinioConn conn) $
